...
export default function AdminView({ user, sectors }: AdminViewProps) {
  ...
  return (
    <div className="text-white font-sans space-y-6">
      ...
      {/* Tab select bar */}
      <div className="flex justify-between items-center bg-[#0a0f21]/60 p-4 border border-white/5 rounded-2xl backdrop-blur-md flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-gray-100 uppercase tracking-widest font-mono">Painel de Administração (RBAC)</h2>
            <p className="text-[10px] text-gray-400">Controles de privilégios de usuários, trilhas de auditoria LGPD e chaves da IA.</p>
          </div>
        </div>

        <div className="flex bg-black/50 p-1.5 rounded-xl border border-white/10 text-xs font-mono">
          <button 
            onClick={() => setActiveTab('users')}
            className={`py-1.5 px-4 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === 'users' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`py-1.5 px-4 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === 'audit' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`py-1.5 px-4 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === 'settings' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
          >
      ...
      <div className="bg-[#0a0f21]/40 border border-white/5 p-6 rounded-2xl min-h-[400px] backdrop-blur-md">
        
        {/* TAB 1: USERS PRIVILEGES LIST */}
        {activeTab === 'users' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-white/5">
              <span className="text-xs font-mono uppercase text-[#00FFB2]">Gerir Cargos & Perfis Admissionais</span>
              <span className="text-[10px] text-gray-500 font-mono">RBAC Enforcement Active</span>
            </div>
                      ...
                      <tr key={emp.id} className="hover:bg-white/[0.02]">
                        <td className="py-3 px-3">
                          <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-full bg-blue-900/40 border border-blue-500/20 text-[#00FFB2] flex items-center justify-center font-bold font-mono">
                              {emp.name.charAt(0)}
                            </div>
                            <div>
                        ...
                        <td className="py-3 px-3 text-blue-300 font-mono">{sectorName.split(" ")[0]}</td>
                        <td className="py-3 px-3">
                          <div className="flex items-center gap-1.5 font-mono text-[10px] text-gray-300">
                            <div className="w-12 h-1.5 bg-black rounded-full overflow-hidden">
                              <div className="h-full bg-[#00FFB2]" style={{ width: `${emp.onboardingProgress}%` }} />
                            </div>
                        ...
                        <td className="py-3 px-3">
                          <select
                            ...
                            className="bg-black/80 border border-white/10 rounded-lg p-1.5 text-[11px] text-white"
                          >
        ...
        {activeTab === 'audit' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-white/5">
              <span className="text-xs font-mono uppercase text-blue-300 flex items-center gap-1.5">
                <Terminal className="w-4 h-4 text-[#00FFB2]" />
                Logs Finais de Auditoria (Rastreabilidade)
              </span>
              <span className="text-[10px] text-gray-500 font-mono">Conformidade LGPD ativa</span>
            </div>

            <div className="space-y-2.5 overflow-y-auto max-h-[400px] pr-2">
              {logs.map((log) => (
                <div key={log.id} className="p-3 bg-[#050505]/45 hover:bg-slate-950/30 rounded-xl border border-white/5 flex flex-wrap justify-between items-center text-xs font-mono gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] bg-blue-950/50 border border-blue-500/20 text-[#00FFB2] px-1.5 py-0.5 rounded">
                        {log.sector}
                      </span>
                      <strong className="text-gray-100">{log.action}</strong>
                    </div>
                    <p className="text-[10px] text-gray-400">Autor: {log.userEmail} • IP Terminal: {log.ip}</p>
                  </div>
              ...
              {logs.length === 0 && (
                <p className="text-center font-mono py-6 text-gray-600">Nenhum evento registrado.</p>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="space-y-5">
            <div className="pb-2 border-b border-white/5">
              <span className="text-xs font-mono uppercase text-[#00FFB2]">Sincronização de Chaves & Motores de IA</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs font-mono">
              <div className="p-4 bg-black/40 rounded-xl border border-white/5 space-y-4">
                <h4 className="text-xs font-bold text-[#00FFB2] uppercase tracking-wider flex items-center gap-1">
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-gray-500">Modelo Selecionado para o Chat RAG</label>
                    <select
                      value={activeModel}
                      onChange={(e) => setActiveModel(e.target.value)}
                      className="w-full bg-[#050505] border border-white/10 rounded-lg p-2.5 text-xs text-white"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-gray-500">Acurácia Semântica Mínima: {ragAccuracyLevel}%</label>
                    <input 
                      type="range"
                      min={80}
                      max={100}
              ...
              <div className="p-4 bg-black/40 rounded-xl border border-white/5 space-y-4">
                ...
                <div className="space-y-3.5">
                  <div className="flex items-center justify-between select-none p-1.5 rounded bg-black/45 hover:bg-slate-900 border border-white/5">
                    <div>
                      <p className="text-gray-200">Exigir MFA Globalmente</p>
                      <p className="text-[9px] text-gray-500">Obrigar todos os novos colaboradores</p>
                    </div>
                    <input 
                      type="checkbox"
                      checked={mfaEnforcedGlobal}
                      onChange={(e) => setMfaEnforcedGlobal(e.target.checked)}
                      className="rounded border-white/20 bg-black text-[#00FFB2] focus:ring-blue-500 w-4 h-4 cursor-pointer"
                    />
                  </div>

                  <div className="p-3 bg-[#050505]/40 border border-blue-500/15 rounded-lg text-blue-300 text-[10px] leading-relaxed">
